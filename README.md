# master_midterm_assignment

- Creating the midterm

<img width="1102" alt="Screenshot 2023-04-20 at 4 42 49 PM" src="https://user-images.githubusercontent.com/90837231/233607154-d63b2a3c-4ffd-40dd-bb79-51c70c1b7653.png">
<img width="1280" alt="Screenshot 2023-04-20 at 5 06 37 PM" src="https://user-images.githubusercontent.com/90837231/233607180-cc0693b6-df7f-44c1-8042-52b6e632e170.png">
<img width="1280" alt="Screenshot 2023-04-20 at 5 06 40 PM" src="https://user-images.githubusercontent.com/90837231/233607215-55a3e333-47f4-4fec-95d6-cf9df98e6402.png">

- Starting roscore


<img width="1280" alt="Screenshot 2023-04-20 at 5 06 43 PM" src="https://user-images.githubusercontent.com/90837231/233607271-18090d4d-e74b-4900-b92f-960de2e1b3c6.png">

- ### Task #1
- checking the midterm services in the new tab
 
<img width="1280" alt="Screenshot 2023-04-20 at 5 06 46 PM" src="https://user-images.githubusercontent.com/90837231/233607304-69bf359d-1975-436c-aaa3-24c8cc31905d.png">

- Testing the service in the new tab

<img width="1280" alt="Screenshot 2023-04-20 at 5 06 49 PM" src="https://user-images.githubusercontent.com/90837231/233607332-b8ef0aab-a777-49b4-9591-05aba5ea0498.png">


### Task #2
- Roslaunch speed_check_simulator.launch
  - speed_limit is pre-defined (80 km/h) and can not be modified
  
  
 <img width="1280" alt="Screenshot 2023-04-21 at 6 52 48 PM" src="https://user-images.githubusercontent.com/90837231/233609331-cc309869-de10-40a6-a7e8-29b8c812de5d.png">


- Testing changing speed 
  - ```plate_number``` is pre-defined (123) and can be modified by ```rosparam set plate_number <new_value>```
  - ```speed``` is pre-defined (60 km/h) and can be modified by ```rosparam set speed <new_value>```
<img width="1280" alt="Screenshot 2023-04-21 at 6 52 53 PM" src="https://user-images.githubusercontent.com/90837231/233609726-1e441db0-22c2-464e-9b4d-d72dfa15b8fc.png">
<img width="1280" alt="Screenshot 2023-04-21 at 6 53 01 PM" src="https://user-images.githubusercontent.com/90837231/233609750-cd65ee8d-2b87-4b92-be05-a700499bde8d.png">



  - Increasing ```speed``` that exceeds ```speed_limit``` (higher than 80) in a new tab
  <img width="1280" alt="Screenshot 2023-04-21 at 6 53 11 PM" src="https://user-images.githubusercontent.com/90837231/233610317-f194942f-541c-400a-88f5-39b3fe978b48.png">

- Now ROS subscriber will echo the plate number of the vehicle

<img width="1280" alt="Screenshot 2023-04-21 at 6 53 18 PM" src="https://user-images.githubusercontent.com/90837231/233609848-c75249da-85bd-44d0-8777-7330afe977cf.png">

 
